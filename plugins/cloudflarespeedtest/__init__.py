    ...
    def __cloudflareSpeedTest(self, event: Event = None):
        ...
        if err_flag:
            ...
            if SystemUtils.is_windows():
                cf_command = f'cd \"{self._cf_path}\" && cfst {self._additional_args} -o \"{self._result_file}\"' + (
            ...
            if SystemUtils.is_windows():
                ...
                if process.poll() is None:
                    os.system('taskkill /F /IM cfst.exe')
            else:
...
def __os_install(self, download_url, cf_file_name, release_version, unzip_command):
        ...
        if Path(f'{self._cf_path}/{cf_file_name}').exists():
            try:
                if SystemUtils.is_windows():
                    ...
                    if Path(f'{self._cf_path}\\cfst.exe').exists():
                ...
                if Path(f'{self._cf_path}/{self._binary_name}').exists() or \
                        Path(f'{self._cf_path}\\cfst.exe').exists():
    ...
    def __get_windows_cfst(self, download_url, proxies):
        response = Response()
        try:
        ...
        if response.status_code == 200:
            with open(f'{self._cf_path}\\cfst_windows_amd64.zip', 'wb') as file:
